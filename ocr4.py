...
def ocr_trocr(img):
    ...
    generated_ids = trocr_model.generate(pixel_values, max_new_tokens=32)
...
for i in range(N):
    ...
    easy=keep_letters(normalize(ocr_easy(binimg)))
    tess=keep_letters(normalize(ocr_tess(binimg)))
    doctr=keep_letters(normalize(ocr_doctr(bgr)))
    trocr=keep_letters(normalize(ocr_trocr(bgr)))
...
print("TrOCR     :",round(np.mean(acc_trocr)*100,2),"%")
